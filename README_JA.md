# DeepCover - フルチェーンコードカバレッジ収集エージェント
...
> JVM Sandbox ベースの Java コードカバレッジ収集ツール - 非侵襲的なラインレベルカバレッジモニタリング

## 概要

DeepCover は、Alibaba JVM Sandbox をベースにした**非侵襲型 Java コードカバレッジ収集エージェント**です。アプリケーションのソースコードを変更することなく、リアルタイムでコード行の実行状況を収集します。

### 主な特徴

- **非侵襲収集** -- JVM Sandbox バイトコード拡張技術に基づき、アプリケーションコードの変更不要
- **ラインレベルカバレッジ** -- コード各行の実行記録を精密に取得
