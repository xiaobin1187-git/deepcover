...
import com.alibaba.jvm.sandbox.api.listener.ext.*;
...
//import static com.alibaba.jvm.sandbox.api.listener.ext.CodeEventWatchBuilder2.PatternType.WILDCARD;
 ...
 */
@Slf4j
public class CodeEventWatchBuilder {
    ...
    public interface IBuildingForBehavior {
         ...
         * @return this
         */
        CodeEventWatcher onWatch(AdviceListener adviceListener);
         ...
         */
        @Deprecated
        CodeEventWatcher onWatch(AdviceListener adviceListener, Event.Type... eventTypeArray);

        CodeEventWatcher onWatch(EventListener eventListener, Event.Type... eventTypeArray);
    ...
    public interface IBuildingForWatching {
         ...
         * @return CodeEventWatcher
         */
        CodeEventWatcher onWatch(CodeAdviceListener adviceListener);
    ...
    private final List<BuildingForClass> bfIgnoreClasses =new ArrayList<>();
     ...
     */
    public CodeEventWatchBuilder(final ModuleEventWatcher moduleEventWatcher) {
    ...
    private static boolean patternMatching(final String string,
        ...
        switch (patternType) {
            ...
            case REGEX:
                ...
                return compiled.matcher(string).matches();
            default:
                return false;
        }
    }
    ...
    public IBuildingForClass ignoreClass(final String pattern) {
        return add(bfIgnoreClasses, new BuildingForClass(pattern));
    }
    ...
    private class BuildingForBehavior implements IBuildingForBehavior {
        ...
        @Override
        public IBuildingForClass onClass(final String pattern) {
            return CodeEventWatchBuilder.this.onClass(pattern);
        }

        @Override
        public IBuildingForClass onClass(final Class<?> clazz) {
            return CodeEventWatchBuilder.this.onClass(clazz);
        }

        @Override
        public IBuildingForClass onAnyClass() {
            return CodeEventWatchBuilder.this.onAnyClass();
        }
        ...
        @Override
        public CodeEventWatcher onWatch(AdviceListener adviceListener) {
            return build(new AdviceAdapterListener(adviceListener), null, BEFORE, RETURN, THROWS, IMMEDIATELY_RETURN, IMMEDIATELY_THROWS);
        }

        @Deprecated
        @Override
        public CodeEventWatcher onWatch(final AdviceListener adviceListener, Event.Type... eventTypeArray) {
            return onWatch(adviceListener);
        }

        @Override
        public CodeEventWatcher onWatch(EventListener eventListener, Event.Type... eventTypeArray) {
    ...
    private class BuildingForWatching implements IBuildingForWatching {
        ...
        @Override
        public CodeEventWatcher onWatch(CodeAdviceListener adviceListener) {
            ...
            return build(
                    new CodeAdviceAdapterListener(adviceListener),
...
//                    if(javaClassName.indexOf(DeepCoverConfig.packageName)==-1||!javaClassName.equals("javax.servlet.http.HttpServlet")){
//                        return false;
//                    }

//                    for (final BuildingForClass bfIgnoreClass : bfIgnoreClasses) {
//
//                        boolean ignore=patternMatching(javaClassName, bfIgnoreClass.pattern, patternType);
//                        if(ignore){
//                            return false;
//                        }
//                    }

//                    if((access & bfClass.withAccess) == bfClass.withAccess
...
//                    }
                    ...
                    if(!patternMatching(javaClassName, bfClass.pattern, patternType)){
                        return false;
                    }
                    if(!DeepCoverConfig.ignoreAnnos.isEmpty()){
                        for(String anno:annotationTypeJavaClassNameArray){
                            if(DeepCoverConfig.ignoreAnnos.indexOf(anno)>-1){
                                return false;
                            }
                        }
                    }
                    boolean result= (access & bfClass.withAccess) == bfClass.withAccess
//                            && patternMatching(javaClassName, bfClass.pattern, patternType)
                            && bfClass.hasInterfaceTypes.patternHas(interfaceTypeJavaClassNameArray);
                   // 不校验注解
//                            && bfClass.hasAnnotationTypes.patternHas(annotationTypeJavaClassNameArray);
                    if(result){
                        ...
                        if(annotationTypeJavaClassNameArray!=null &&annotationTypeJavaClassNameArray.length>0){
                            StringBuilder stringBuilder = new StringBuilder();
                            stringBuilder.append(javaClassName).append(":");
                            for(String anno:annotationTypeJavaClassNameArray){
                                stringBuilder.append(anno).append(":");

                            }
                            log.info("类注解信息:{}",stringBuilder.toString());
                        }
                ...
                public boolean doMethodFilter(final int access,
                    ...
                    if(!DeepCoverConfig.ignoreMethods.isEmpty()){
                        String[] ignoreMethods = DeepCoverConfig.ignoreMethods.split(";");
                        for(String method:ignoreMethods){
                            if(method.equals(javaMethodName) || patternMatching(javaMethodName, method, patternType)){
                                return false;
                            }
                        }
                    }
    ...
    private CodeEventWatcher build(final EventListener listener,
